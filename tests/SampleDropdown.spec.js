...
test('dropdown', async({page})=>{
    ...
    // Assertions
    //1. Check no of options available in dropdown --  method 1 
    // const option = await page.locator('#country option');
    // await expect(option).toHaveCount(10);


    //2. Check no of options available in dropdown --  method 2

    //const option = await page.$$('#country option');
    //console.log(option.length)
    //await expect(option.length).toBe(10)

    //3. Check the presence of options available in dropdown

    const content = await page.locator('#country').textContent()
    await expect(content.includes('Poland')).toBeFalsy();
