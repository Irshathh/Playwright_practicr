...
- Name: assertion2.spec.js >> All Assertions Practice
...
```ts
  ...
  2  | 
  3  | test('All Assertions Practice', async ({ page }) => {
  4  | 
  ...
  19 |     const nameInput = page.getByPlaceholder('Name');
  20 |     await expect(nameInput).toBeEnabled();
  21 | 
  22 |     // 5) toBeChecked (Radio)
  23 |     const radioBtn = page.locator('#id_gender1');
> 24 |     await radioBtn.check();
  ...
  31 | 
  32 |     // 6) toHaveText
  33 |     const heading = page.locator('h2');
  34 |     await expect(heading.first()).toHaveText(/New User Signup!/);
  35 | 
  36 |     // 7) toContainText
  37 |     await expect(heading.first()).toContainText('Signup');
  38 | 
  39 |     // 8) toHaveValue
  40 |     await nameInput.fill('Irshad');
  41 |     await expect(nameInput).toHaveValue('Irshad');
  42 | 
  43 |     // 9) toHaveCount
  44 |     const menuItems = page.locator('.shop-menu ul li');
  45 |     await expect(menuItems).toHaveCount(8); // may vary slightly
  46 | 
  47 | });
```