...
```ts
  ...
  38 |     constructor(page){
  39 |         this.page = page;
  40 |         this.productList = '//div[@id="tbodyid"]//div//h4//a'
  41 |         this.addToCartBtn = '//a[text()="Add to cart"]'
  42 |         this.cart = '#cartur';
  43 |     };
  44 | 
  45 |     async addProducts(ProductName){
> 46 |         const ProductLists = await this.page.$$(this.productList)
     |                                              ^ Error: page.$$: Execution context was destroyed, most likely because of a navigation
  47 |         for (const product of ProductLists){
  48 |             if (ProductName === await product.textContent() ){
  49 |                 await product.click()
  50 |                 break;
  51 |             }
  52 |         }
  53 |         await this.page.on('dialog',async dialog=>{
  54 |             if(dialog.message().includes('Product added')){
  55 |                 await dialog.accept()
  56 |             }
  57 |         });
  58 | 
  59 |         await this.page.locator(this.addToCartBtn).click()
  60 |     };
  61 | 
  62 |     async goToCart(){
