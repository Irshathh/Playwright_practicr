...
- Location: tests\pom.spec.js:45:1

# Error details

```
TypeError: product.textcontent is not a function
```
    ...
    - list [ref=e6]:
      ...
      - listitem [ref=e16]:
        - link "Log out" [ref=e17] [cursor=pointer]:
          - /url: "#"
      - listitem [ref=e18]:
        - link "Welcome irshath" [ref=e19] [cursor=pointer]:
          - /url: "#"
      - listitem
    - generic [ref=e21]:
      - list [ref=e22]:
...
```ts
  ...
  35 |     constructor(page){
  36 |         this.page =page;
  37 |         this.productList = '//div[@id ="tbodyid"]//div//h4//a';
  38 |         this.addToCartBtn= '//a[text()="Add to cart"]'
  39 |         this.cart = '#cartur';
  40 |     };
  41 | 
  42 |     async addProducts(productName){
  43 |         const ProductLists = await this.page.$$(this.productList);
  44 |         for (const product of ProductLists){
> 45 |             if(productName === await product.textcontent() ){
     |                                              ^ TypeError: product.textcontent is not a function
  ...
  49 |         }
  50 | 
  51 |         await this.page.on('dialog', async dialog=>{
  52 |             if (dialog.message().includes('Product added')){
  53 |                 await dialog.accept()
  54 |             }
  55 |         });
  56 | 
  57 |         await this.page.locator(this.addToCartBtn).click()
  58 |  
  59 | 
  60 | 
  61 |     };
  62 |     async goToCart(){
  63 |         await this.page.locator(this.cart).click()
  64 |     }
  65 | }
  66 | 
  67 | 
