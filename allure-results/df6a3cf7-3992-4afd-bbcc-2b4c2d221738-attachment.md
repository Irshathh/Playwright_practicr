...
```
Error: locator.fill: Test timeout of 1000ms exceeded.
Call log:
  - waiting for locator('[id="loginpassword"]')
...
```ts
  ...
  6  |         this.userNameInput = '//input[@id="loginusername"]';
  7  |         this.passwordInput = '[id="loginpassword"]';
  8  |         this.loginBtn = '//button[text()="Log in"]';
  9  |     }
  10 | 
  11 |     async gotoUrl() {
  ...
  16 |         await this.page.locator(this.loginLink).click();
  17 |         await this.page.locator(this.userNameInput).fill(userName);
> 18 |         await this.page.locator(this.passwordInput).fill(passWord);
     |                                                     ^ Error: locator.fill: Test timeout of 1000ms exceeded.
  19 |         await this.page.locator(this.loginBtn).click();
  20 |     }
  21 | }
```