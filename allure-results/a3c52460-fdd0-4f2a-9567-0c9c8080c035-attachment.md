...
```
ReferenceError: breakl is not defined
```
...
- generic [ref=e1]:
  ...
  - generic [ref=e12]:
    ...
    - group "Suggession Class Example" [ref=e23]:
      ...
      - textbox "Type to Select Countries" [active] [ref=e25]: Argentina
...
```ts
  ...
  42 | 
  43 |     const CountryOption = await page.$$('//li[@class="ui-menu-item"]//div');
  44 | 
  45 |     for (let option of CountryOption){
  46 |         const value = await option.textContent();
  47 |         console.log(value);
  48 |         if (value.includes('Argentina')){
