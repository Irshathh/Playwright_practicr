...
- Name: softassertion.spec.js >> Assertion
- Location: tests\softassertion.spec.js:3:5
...
Received: "https://demoblaze.com/"
Timeout:  5000ms

Call log:
  - Expect "soft toHaveURL" with timeout 5000ms
    9 × unexpected value "https://demoblaze.com/"
...
```ts
  ...
  6  | 
  7  | 
  8  |     await expect.soft(page).toHaveTitle('STORE123');
> 9  |     await expect.soft(page).toHaveURL('https://www.demoblaze.com/index.html');
     |                             ^ Error: expect(page).toHaveURL(expected) failed
  10 |     await expect.soft(page.locator('.navbar-brand')).toBeVisible();
  11 | })
```