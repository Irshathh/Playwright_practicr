...
```ts
  ...
  6  | 
  7  | 
> 8  |     await expect(page).toHaveTitle('STORE123');
     |                        ^ Error: expect(page).toHaveTitle(expected) failed
  9  |     await expect(page).toHaveURL('https://www.demoblaze.com/index.html');
  10 |     await expect(page.locator('.navbar-brand')).toBeVisible();
  11 | })
```